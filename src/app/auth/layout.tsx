...
import { UserStatusProvider } from '../providers/UserStatusProvier';


export default async function AuthLayout({ children }: { children: ReactNode }) {
  const session = await getServerSession(authOptions);

  const userStatus = await userExistStatus(session?.user.email ?? undefined);
  return (
    <UserStatusProvider userStatus={userStatus}>
