...
function UserSchool(
  { initialProfile }: { initialProfile: IProfilePopulated }
) {
  return (
  ...
  />  
    ...
    { (initialProfile?.experiences[0]&& initialProfile?.experiences[0]?.positions[0]?.title) || (initialProfile?.educations[0] && initialProfile?.educations[0]?.school) || "No School or Experience"
      }
      </p>
  </div>
  )
